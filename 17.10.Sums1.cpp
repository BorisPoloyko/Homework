...
char inputChoice();
int inputParameter();
int inputParameter(int minVal, int maxVal);
...
double factorial(int n) //я не смог придумать способ, как здесь параметром сделать результат функции inputParameter(int minVal, int maxVal);
{
	if (n == 0 || n == 1)
	{
		return 1;
...
int inputParameter(int minVal, int maxVal)
{
	...
	cout << "Enter the parameter 'n' for the following expressions\nn= ";
	cin >> n;
	while ((n != floor(n) || n < minVal) && n>maxVal)
	{
